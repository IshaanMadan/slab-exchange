...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  createCard(
    ...
    return this.fb.group({
      ...
      form_group: this.fb.group({
        ...
        auto_grade: [auto_grade, Validators.required],
  ...
  removeCard(index) {
    ...
    if(this.cards.at(index).get('card_id').value) {
      this.dataService.deleteCard(this.cards.at(index).get('card_id').value)
      .subscribe((response:any ) => {
        if(response.success) {
  ...
  enableDisableForm(index) {
    ...
    if(card.get(['image_group', 'back_image']).value != null && card.get(['image_group', 'front_image']).value != null) {
  ...
  imageUploaded($event, index, is_front) {
    ...
    this.dataService.uploadImage(fromData, is_front, card_id)
    .subscribe((response: any) => {
      if(is_front) {
  ...
  updateCardData(index) {
    ...
    this.dataService.saveCardDetails(cardData)
    .subscribe(response => {
