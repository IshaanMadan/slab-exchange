...
export class CollectionsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    .subscribe((response: any) => {
      ...
      this.collectionList.forEach(element => {
        ...
        element['certification_label'] = this.findCertificationLabel(element.certification)
        element['card_grade_label'] = this.findCardGradeLabel(element.card_grade)
        element['auto_grade_label'] = this.findAutoGradeLabel(element.auto_grade)
      });
    }, error => {
      this.collectionList = [];
    })
  ...
  imageUploaded($event, index, is_front) {
    ...
    this.dataService.uploadImage(fromData, is_front, card_id).subscribe(
      (response: any) => {
        if (is_front) {
          this.collectionList[index].front_thumbnail = response.data.front_thumbnail;
        } else {
          this.collectionList[index].back_thumbnail = response.data.back_thumbnail;
        }
      },
      (error) => {}
    );
  ...
  updateCardData(index) {
    ...
    cardData['certification_number'] = this.collectionList[index].certification_number
    cardData['autographed'] = this.collectionList[index].autographed ? true : false;
